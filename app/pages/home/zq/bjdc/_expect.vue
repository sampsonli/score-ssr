<template>
    <div class="l-full l-flex-column">
        <div class="qi-list-box">
            <div class="qi-list">
                <ul class="responsive">
                    <li class="" v-if="preAndNextExpect.pre"><nuxt-link :to="{path: '/home/zq/bjdc/'+ preAndNextExpect.pre}" replace>前一期</nuxt-link></li>
                    <li class="" v-else>前一期</li>
                    <li class="qiqh" v-tap="{methods: toggleExpectList}">{{curExpect}} 期<i class="qi-arrow"></i></li>
                    <li class="" v-if="preAndNextExpect.next"><nuxt-link :to="{path: '/home/zq/bjdc/'+ preAndNextExpect.next}" replace>后一期</nuxt-link></li>
...
<script>
    ...
    export default {
        fetch ({store, params}) {
            if (store.state.home.zq.bjdc.allMatches[params.expect]) {
                return Promise.resolve()
            } else {
                return store.dispatch('home/fetchBjdcMatches', params.expect)
            }
        },
        data () {
            return {

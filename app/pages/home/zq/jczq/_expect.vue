<template>
    <div class="l-full l-flex-column">
        <div class="qi-list-box">
            <div class="qi-list">
                <ul class="responsive">
                    <li class="" v-if="preAndNextExpect.pre"><nuxt-link :to="{path: '/home/zq/jczq/'+ preAndNextExpect.pre}" replace>前一期</nuxt-link></li>
                    <li class="" v-else>前一期</li>
                    <li class="qiqh" v-tap="{methods: toggleExpectList}">{{curExpect}} 期<i class="qi-arrow"></i></li>
                    <li class="" v-if="preAndNextExpect.next"><nuxt-link :to="{path: '/home/zq/jczq/'+ preAndNextExpect.next}" replace>后一期</nuxt-link></li>
    ...
    export default {
        fetch ({store, params}) {
            if (store.state.home.zq.jczq.allMatches[params.expect]) {
                ...
                return store.dispatch('home/fetchJczqMatches', params.expect)
            }
        },
        data () {
            return {
        ...
        computed: {
            matchInfo () {
                return this.$store.state.home.zq.jczq
            },
            matches () {
        ...
        methods: {
            ...
            selectExpect ({expect}) {
                this.$router.replace(`/home/zq/jczq/${expect}`)
            }
        }
    }
</script>

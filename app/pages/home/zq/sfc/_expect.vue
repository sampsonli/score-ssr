...
<script>
    ...
    export default {
        fetch ({store}) {
            if (store.state.home.zq.sfc.allMatches[store.state.home.zq.sfc.curExpect]) {
                return Promise.resolve()
            } else {
                return store.dispatch('home/fetchSfcMatches')
            }
        },
        components: {
            MatchesScroller
        ...
        computed: {
            matches () {
                return this.$store.state.home.zq.sfc.allMatches[this.$store.state.home.zq.sfc.curExpect]
            }
        },
        methods: {
            goDetail () {

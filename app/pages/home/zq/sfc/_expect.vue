            ...
            <div class="qi-list">
                <ul class="responsive">
                    <li class="" v-if="preAndNextExpect.pre">
                        <nuxt-link :to="{path: '/home/zq/sfc/'+ preAndNextExpect.pre}" replace>前一期</nuxt-link>
                    </li>
                    ...
                    <li class="" v-if="preAndNextExpect.next">
                        <nuxt-link :to="{path: '/home/zq/sfc/'+ preAndNextExpect.next}" replace>后一期</nuxt-link>
                    </li>
...
<script>
    ...
    export default {
        async asyncData ({store, params}) {
            let allMatches = store.state.home.zq.sfc.allMatches
            let {expect} = params
            expect = expect || 'cur'
            if (allMatches[expect]) {
                ...
                let { matches } = await store.dispatch('home/fetchSfcMatches', expect)
                return {
